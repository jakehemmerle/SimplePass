...
class CryptoEngine:
    def __init__(self, secret, database_location=None):
        ...
        self.cipher = AES.new(key, AES.MODE_GCM)    # nonce gets set later
    ...
    @staticmethod
    def _pad(s):
        # pads them cleartexts
        return s + b"\0" * (AES.block_size - len(s) % AES.block_size)

    def _get_ciphertext_and_tag(self, plaintext):
    ...
    def _decode_blob(blob):
        ...
        return binascii.unhexlify(message['ciphertext']), binascii.unhexlify(message['tag']), binascii.unhexlify(message['nonce'])
